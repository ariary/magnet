...
func SendFileHTTP(client *http.Client, endpoint string, filename string) (codeerr error) {
	// file exists?
	...
	err = UploadHTTP(client, endpoint, values)
	if err != nil {
		return err
	}
...
// UploadHTTP: construct upload request and sumbit it
func UploadHTTP(client *http.Client, url string, values map[string]io.Reader) (err error) {
